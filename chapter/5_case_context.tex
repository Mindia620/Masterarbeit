...
This chapter provides a comprehensive description of the organizational context in which the artifact developed in this thesis is based on.
It presents EnterpriseX, a large, family-owned European industrial company, as the base for the \ac{DSR} artifact.
It touches the historical IT structure from which the transition departs, the strategic drivers that triggered the centralization initiative, the current state of the transition across eight separate IT domains and the main governance challenges that the governance model must address.
The enterprise was selected as the context base because it exhibits precisely the structural and governance characteristics that motivate this research.
A detailed justification of this selection is provided in Section \ref{subsec:CaseSelectionAndContextOverview}.

%compare this chapter to chapter 4 because some topics are repeated too much and should be worked on in reference to each other instead of repeating myself

\subsection{Organizational Background - check for less detail}
\label{subsec:OrganizationalBackground}
EnterpriseX is a large, family-owned international group headquartered in Germany and operating in the  economy sector.
...
With more than 10,000 employees, over 200 processing sites and active operations across all countries, EnterpriseX represents a sizeable and organizationally complex enterprise.
\\
The organizational structure of EnterpriscirculareX is layered and reflects the group's growth through acquisition and regional expansion.
The central holding entity fulfills a service and coordination function, as it is providing group-level shared services, financial, legal and further services, while the actual operational business is conducted by a set of legally and operationally distinct daughter companies.
These companies are responsible for a specific segment of the business or geographic region.
...
\\
This decentralized organizational logic historically extended to the IT setup: each country subsidiary operated its own IT function, reporting to local management rather than to any group-level IT authority.
